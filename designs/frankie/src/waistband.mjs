...
export const waistband = {
  ...
  draft: (sh) => {
    const { macro, store, points, paths, Path, options, measurements, complete, snippets, Snippet, absoluteOptions, part } = sh
    store.set('waistbandLength', (store.get('waistBack') + store.get('waistFront')) * 2)
    store.set('waistbandLengthTop', store.get('waistbandLength') -
    (absoluteOptions.waistbandWidth * (measurements.seat * (1 + options.seatEase) -  measurements.waist * (1 + options.waistEase))) /
  ...
  store.cutlist.setCut({ cut: 2, from: 'fabric', identical: 'true' })
    ...
    macro('sprinkle', {
      snippet: 'notch',
      on: ['bottomLeftNotch', 'bottomMid', 'bottomRightNotch', 'bottomRight', 'topRight', 'topRightNotch', 'topMid', 'topLeftNotch'],
    })
    if (complete) {
