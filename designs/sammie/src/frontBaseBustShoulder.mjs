...
export const frontBaseBustShoulder = {
  name: 'sammie.frontBaseBustShoulder',
  after: backBase,
  draft: (sh) => {
    const {
      store,
    ...
    let neckSideFrontCpFraction =
      (options.sideFrontCurve * points.shoulder.dist(points.bustDartBottom)) /
      points.hps.dist(points.shoulderAnchor)
