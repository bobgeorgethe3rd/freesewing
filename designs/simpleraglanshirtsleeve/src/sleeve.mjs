...
import { sleeve as basicraglansleeve } from '@freesewing/basicraglansleeve'

export const sleeve = {
  name: 'simpleraglanshirtsleeve.sleeve',
  measurements: [...basicraglansleeve.measurements],
  options: {
    //Imported
    ...basicraglansleeve.options,
    //Constants
  ...
  draft: (sh) => {
    ...
    //draft
    basicraglansleeve.draft(sh)
    //remove paths
    const keepThese = ['sleeveCapFront', 'neck', 'sleeveCapBack', 'grainline']
    ...
    if (complete) {
      ...
      if (sa) {
        ...
        points.saSleeveCapRight = new Point(
          drawSaRight().end().x,
          paths.sleeveCapFront.offset(sa).start().y
        )
        points.saSleeveCapLeft = new Point(
          drawSaLeft().start().x,
          paths.sleeveCapBack.offset(sa).end().y
        )
