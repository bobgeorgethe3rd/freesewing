export const frontSideDart = ({
  ...
  if (options.bodyLength == 0 && options.bustDartFraction > 0.994) {
    ...
    if (points.bustDartBottom.y < points.bustDartBottomI.y) {
      paths.sideSeamBottom.line(points.bustDartBottom).hide()
    } else {
      paths.sideSeamBottom = paths.sideSeamBottom
        .line(points.armholeDrop)
  ...
  if (points.bustDartTop.y > points.bustDartTopI.y) {
  ...
  if (complete) {
    ...
    if (sa) {
      ...
      if (options.bustDartFraction == 0) {
        paths.saSideSeamTopSa = new Path()
          .move(
            points.armholeDrop
