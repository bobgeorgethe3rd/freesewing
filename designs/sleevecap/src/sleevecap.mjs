...
function draftSleevecap(part, run) {
  ...
  points.bicepsLeft = points.centerBiceps.shift(
    180,
    halfWidth * options.sleevecapWidthGuarantee +
      halfWidth * (1 - options.sleevecapWidthGuarantee) * store.get('sleeveFactor')
  )
...
export const sleevecap = {
  ...
  draft: ({
    ...
    } while (options.fitSleevecap === true && run < 50 && Math.abs(sleevecapDelta(store)) > 2)
